...
namespace Evento.Infrastructure.Services
{
    public class TicketService : ITicketService
    {
        ...
        public async Task<IEnumerable<TicketDetailsDto>> GetTicktesForUserAsync(Guid userId)
        {
            ...
            {
                var tickets = _mapper.Map<IEnumerable<TicketDetailsDto>>(_event.GetTicketsPurchasedByUser(user)).ToList();
                tickets.ForEach(t =>
                {
                    t.EventId = _event.Id;
                    t.EventName = _event.Name;
                });
        ...
        public async Task CancelTicketAsync(Guid userId, Guid eventId, int amount)
        {
            var user = await _userRepository.GetOrFailAsync(userId);
        ...
        public async Task PurchaseTicketsAsync(Guid userId, Guid eventId, int amount)
        {
            var user = await _userRepository.GetOrFailAsync(userId);
            var @event = await _eventRepository.GetOrFailAsync(eventId);
